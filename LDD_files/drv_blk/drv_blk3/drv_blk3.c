...
struct my_blk_dev
{
     int size;                       // Disk size in units of block/sectors
     u8 *data;                       // Data buffer for emulating RAM Disk   
     short users;                    // Number of current users of the disk
     short media_change;             // Flag for tracking change in disk content
     spinlock_t lock;                // For mutually exclusive access of the disk
     ...
     struct timer_list timer;        // For simulated media (disk) change event
};
...
#define INVALIDATE_DELAY	30*HZ // 30 Seconds
...
static void drv_blk_media_change(struct timer_list *tm)
{
    printk(KERN_INFO "drv_blk_media_change()\n");
...
{
    struct my_blk_dev *bdev;

    printk(KERN_INFO "drv_blk_open()\n");

    bdev = (struct my_blk_dev *)(bd->bd_disk->private_data);
    del_timer_sync(&(bdev->timer)); // Delete revalidation timer, if running.
...
static void drv_blk_release (struct gendisk *gd, fmode_t fmode)
{
    ...
    bdev = (struct my_blk_dev *)(gd->private_data);

    spin_lock(&bdev->lock);
    bdev->users--;
    if (!bdev->users)
    {
        // Add revalidation timer.
        bdev->timer.expires = jiffies + INVALIDATE_DELAY;
        timer_setup(&(bdev->timer), drv_blk_media_change, 0);
...
static int drv_blk_ioctl (struct block_device *bd, fmode_t fmode, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
         ...
         * since RAM disk is a virtual device, we have to provide fake data
         * about the geometry of the disk in order to satisfy the system.
         * So we claim 16 sectors, four heads and calculate the corresponding
         * number of cylinders. We set the start of data at sector number four.
         */
        size = bdev->size*(hardsect_size/KERNEL_SECTOR_SIZE);
...
static int drv_blk_media_changed (struct gendisk *gd)
{
    struct my_blk_dev *bdev;

    printk(KERN_INFO "drv_blk_media_changed()\n");
...
static int drv_blk_revalidate_disk (struct gendisk *gd)
{
    struct my_blk_dev *bdev;

    printk(KERN_INFO "drv_blk_revalidate_disk()\n");

    bdev = (struct my_blk_dev *)(gd->private_data);
    if (bdev->media_change)
    {
        bdev->media_change = 0;
        memset (bdev->data, 0, bdev->size);
    }
...
static struct block_device_operations blk_dev_ops = {
    ...
    .ioctl = drv_blk_ioctl,
    .media_changed = drv_blk_media_changed,
    .revalidate_disk = drv_blk_revalidate_disk
};
...
static int __init drv_blk_init (void)
{
        ...
        snprintf (bdev->gd->disk_name, 32, "cgrd%c", 'c');
