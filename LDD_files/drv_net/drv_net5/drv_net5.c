...
static void drv_net_hw_tx(char *data, int len, struct net_device *txdev)
{
    ...
    nd_priv->tx_packetlen = len; // Save the packet length to be used for stats.
...
// The virtual interrupt handler (for virtual device in this case).
static void drv_net_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
    int int_status;
    ...
    printk(KERN_INFO "drv_net_interrupt() called\n");
