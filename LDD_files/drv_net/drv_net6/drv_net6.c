...
#define DRV_NET_RX_INTR 0x0001
#define DRV_NET_TX_INTR 0x0002
...
struct net_dev_priv
{
    ...
    unsigned char *tx_packetdata; // Save raw data of current packet in process
    int tx_packetlen; 		// Save length of raw data of current packet
    struct timer_list watchdog_timer;	// Timer for transmission timeout
    int rx_int_enabled;		// Rx interrupt enabled/disabled flag
    int int_status;		// Rx interrupt status flags
    ...
    struct napi_struct napi;	// New API structure
    int phy_reg[MAX_PHY_REG];	// Simulate PHY Registers.
};

// NAPI mode
static int napi_mode = 0;
module_param(napi_mode, int, 0);

// Create two instances of Virtual Network Device (VND) 
struct net_device *drv_net_devs[2];

// Function pointer to select normal and NAPI interrupt handler
static void (*drv_net_interrupt)(int, void *, struct pt_regs *);
...
static void drv_net_hw_tx(char *data, int len, struct net_device *txdev)
{
    ...
    /*
     * Now the packet is ready for transmission. First simulate a
     * receive interrupt on the twin (receiving) device, then a
     * transmit interrupt on the transmitting device.
     */
    ...
    /*
     * At this point in time, incoming packet is supposed to have been
     * received, hence simulate Rx interrupt condition.
     */
    if (nd_priv->rx_int_enabled)
    {
        nd_priv->int_status |= DRV_NET_RX_INTR;
        drv_net_interrupt(0, rxdev, NULL);
    }

    /*
     * At this point in time, outgoing packet is supposed to have been
     * transmitted, hence simulate Tx interrupt condition.
     */
    nd_priv = netdev_priv(txdev);
    nd_priv->tx_packetlen = len;
    nd_priv->tx_packetdata = data;
    nd_priv->int_status |= DRV_NET_TX_INTR;
    drv_net_interrupt(0, txdev, NULL);
}
...
static netdev_tx_t drv_net_tx(struct sk_buff *skb, struct net_device *dev)
{
    ...
    // Remember the skb, so we can free it at interrupt processing time.
    nd_priv->skb = skb;
...
static void drv_net_tx_timeout (struct timer_list *t)
{
    ...
    // Simulate a Tx interrupt to get things moving.
    nd_priv->int_status = DRV_NET_TX_INTR;
    drv_net_interrupt(0, dev, NULL);
...
static void drv_net_rx_ints(struct net_device *dev, int enable)
{
    struct net_dev_priv *nd_priv = netdev_priv(dev);
    nd_priv->rx_int_enabled = enable;

    return;
}

// NAPI polling implementation
static int drv_net_poll(struct napi_struct *napi, int weight)
{
    // Retrieve struct net_dev_priv from struct napi_struct.
    struct net_dev_priv *nd_priv = container_of(napi, struct net_dev_priv, napi);
    struct net_device *dev = nd_priv->dev;
    int work_done = 0;
    struct sk_buff *skb;
    struct net_pkt_buffer *pkt_buffer;

    printk(KERN_INFO "drv_net_poll() called\n");

    while (work_done < weight && nd_priv->rx_q) 
    {
        // Remove the packet buffer from Rx queue/list.
        pkt_buffer = drv_net_dequeue_pkt_buffer(dev);

        // Allocate additional 2-bytes for IP header alignment.
        skb = dev_alloc_skb(pkt_buffer->datalen + 2);
        if (!skb)
        {
            printk(KERN_NOTICE "drv_net_poll(): No memory - packet dropped\n");
                    nd_priv->stats.rx_dropped++;
                    drv_net_release_pkt_buffer(pkt_buffer);
            continue;
        }
        // Align IP header on 4-byte address boundary.
        skb_reserve(skb, 2);

        // copy the packet buffer data to newly allocated SKB.
        memcpy(skb_put(skb, pkt_buffer->datalen), pkt_buffer->data, pkt_buffer->datalen);

        // Fill up additional information into SKB.
        skb->dev = dev;
        skb->protocol = eth_type_trans(skb, dev);
        skb->ip_summed = CHECKSUM_UNNECESSARY; /* don't check it */

        // Inject the packet (SKB) to network stack.
        ...
        // Release the packet buffer to the packet buffer pool/queue.
        drv_net_release_pkt_buffer(pkt_buffer);
    }

    /* 
     * If we processed all packets, we're done.
     * Tell the kernel and re-enable the interrupts.
     */
    if (! nd_priv->rx_q)
    {
        napi_complete_done(&nd_priv->napi, work_done);
        drv_net_rx_ints(dev, 1);
        return 0;
    }
...
// The normal interrupt handler (for virtual device in this case).
static void drv_net_normal_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
    int int_status;
    struct net_pkt_buffer *pkt_buffer = NULL;
    struct net_device *dev = (struct net_device *)dev_id;
    struct net_dev_priv *nd_priv = netdev_priv(dev);

    printk(KERN_INFO "drv_net_normal_interrupt() called\n");

    if (!dev)
        return;

    // Lock the device.
    spin_lock(&nd_priv->lock);

    // Retrieve int_status: real devices use I/O operation.
    int_status = nd_priv->int_status;
    nd_priv->int_status = 0;

    /*
     * In real device driver, check with device hw if this interrupt is ours.
     * If it is our interrupt, process it further. 
     */

    // Process Rx interrupt.
    if (int_status & DRV_NET_RX_INTR)
    {
        /*
         * Remove a packet from Rx queue for processing.
         * Do not use drv_net_dequeue_pkt_buffer() function here as spinlock
         * has already been aquired in this function. Calling
         * drv_net_dequeue_pkt_buffer() will result in has it tries to 
         * aquire the spinlock again. 
         */
        pkt_buffer = nd_priv->rx_q;
        if (pkt_buffer)
        {
            nd_priv->rx_q = pkt_buffer->next;
            // Send the packet buffer to drv_net_rx() for handling.
            drv_net_rx(dev, pkt_buffer);
        }
    }

    // Process Tx interrupt.
    if (int_status & DRV_NET_TX_INTR)
    {
        /*
         * Packet transmission is over.
         * Update the network statistics and free the SKB.
         */
        nd_priv->stats.tx_packets++;
        nd_priv->stats.tx_bytes += nd_priv->tx_packetlen;
    ...
    // Unlock the device.
    spin_unlock(&nd_priv->lock);

    /*
     * Release the packet buffer to packet buffer pool/queue.
     * This should be done outside the lock as drv_net_release_pkt_buffer()
     * also acquires the spinlock.
     */
    if (pkt_buffer)
        drv_net_release_pkt_buffer(pkt_buffer);
...
static void drv_net_napi_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
    int int_status;
    struct net_dev_priv *nd_priv;
    struct net_device *dev = (struct net_device *)dev_id;

    printk(KERN_INFO "drv_net_napi_interrupt() called\n");

    if (!dev)
        return;

    // Lock the device.
    nd_priv = netdev_priv(dev);
    spin_lock(&nd_priv->lock);
    ...
    if (int_status & DRV_NET_RX_INTR) 
    {
        ...
        drv_net_rx_ints(dev, 0);

        // Schedule the packet processing - calls drv_net_poll().
        napi_schedule(&nd_priv->napi);
    }
...
static void drv_net_setup(struct net_device *dev)
{
    ...
    // Initialize NAPI mode of operation.
    netif_napi_add(dev, &nd_priv->napi, drv_net_poll, 64);
...
static int __init drv_net_init(void)
{
    ...
    drv_net_interrupt = napi_mode ? drv_net_napi_interrupt : 
				    drv_net_normal_interrupt;
